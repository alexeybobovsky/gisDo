...
{else}
<td class='slider-value'><div id='productType_{$curLvl}_Label' style='display:;'>{$prdctLabel}</div></td>
<td class='slider-input'>
<select name="productType_{$curLvl}" id="productType_{$curLvl}"  onChange='productTypeSelect_Add(this);  check4LabelVisibility(this);'
	data-placeholder="Выберите {$prdctLabel}" class="chzn-select" tabindex="-1" style="width: 300px;" >	
	{section   name=prdGrp loop=$prdctGroups}
	<option value="{$prdctGroups[prdGrp].id}">{$prdctGroups[prdGrp].name}</option>	

	{/section}
	<option disabled value=""></option>
	<option value="new" style='color:grey;'>Добавить {$prdctLabel} >></option>
	</select> 
	
	<span id='waitImgCont'>	<img id='waitImg_{$curLvl}' border=0 src='/src/design/main/_.gif'></span>
