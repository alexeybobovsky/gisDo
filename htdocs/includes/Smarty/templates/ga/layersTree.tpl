...
{section name=tree loop=$tree}	
	...
	<tr id = 'tr_{$SMRT_NODE.item.ot_id}' >
		<td>
	{if $SMRT_NODE.isFirst}
		<TABLE border=0 {if $SMRT_NODE.lvl>0} style="DISPLAY: none" {/if} 
			id='table_{$SMRT_NODE.item.ot_id}' 
			cellSpacing=0 cellPadding=0 width="100%" border=0 >
		  <TBODY id='body_{$SMRT_NODE.item.ot_id}' >
		<tr id='table_tr{$SMRT_NODE.item.ot_id}' >
			<td valign='middle'>			
	{/if}
	<div id='node_{$SMRT_NODE.item.ot_id}'>
	...
	<IMG class=tree src="/src/design/tree/treePlus.gif" width=16 align=middle 
			id = 'img_{$SMRT_NODE.item.ot_id}'
			onclick="treeExpand(this, 'table_{$tree[$smarty.section.tree.index_next].item.ot_id}')"
	...
	{/if}
	<span id=item_{$SMRT_NODE.item.ot_id}>
					<span  id=link_{$SMRT_NODE.item.ot_id} 					
					{*if $Tree.admin*}
					onclick="itemSelectLayer(this); "    	  
					onMouseMove="this.style.cursor='hand'; return false;"
					{*/if*}>
					...
					<font color='gray'>
					{/if*}
						{$SMRT_NODE.item.ot_name|truncate:80:"...":true}
				...
				<span id='empty_{$SMRT_NODE.item.ot_id}'></span>
	</div>
		</td>
	</tr>	
