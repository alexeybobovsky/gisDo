...
{section name=sel loop=$SMRT_EL.value}	
	<option value="{$SMRT_EL.value[sel]}" {if !$SMRT_EL.multiple && $SMRT_EL.default == $SMRT_EL.value[sel]} selected 
										{elseif $SMRT_EL.multiple && $SMRT_EL.default[sel]}selected{/if} >{$SMRT_EL.caption[sel]}</option>
{/section}
</select>					