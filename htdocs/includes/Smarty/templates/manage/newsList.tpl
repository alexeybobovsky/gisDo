...
{else}
      <TABLE cellSpacing=0 cellPadding=1 border=0 id = '{$orders.table.body_id}' class='{*Order1-1*}din' width='99%' >
	  <THEAD>
		<TR>
  ...
  <TBODY>
	{section name=cats loop=$orders.content}
		<tr style='background:{cycle name="tr" values="#ffffff,#f8f8f8"}'  
			...
			id='link_{$orders.content[cats].id}'
		...
		<td align='{$orders.align[cats].$SMRT_col}' valign='middle'>
			...
			<span class='disabled'>
			{/if}
			{if $smarty.section.cols.index == 0 && $orders.content[cats].doc2catCnt}			
			<img src='/src/design/admin/chain.gif' border='0' title='Связанная статья'>
			{else}			
			{/if}						
			{if $orders.content[cats].$SMRT_col}<nobr>{$orders.content[cats].$SMRT_col}</nobr>{else}&nbsp;{/if}				
			{if $orders.content[cats].hidden}
			</span >
			{/if}
		</td>		
