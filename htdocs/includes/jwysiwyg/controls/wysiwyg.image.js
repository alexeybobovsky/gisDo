	...
	$.wysiwyg.controls.image = {
		...
		init: function (Wysiwyg) {
			...
			formImageHtml = '<form class="wysiwyg" id="wysiwyg-addImage"><fieldset>' +
				'<div class="form-row" style="margin-bottom:20px; color: #c66;">Для публикации Ваших фотографий на сайте fotostroek.ru нужно предварительно загрузить их на сторонние площадки и вставить адрес изображения в соответствующее поле. Вы можете воспользоваться такими  сервисами фотохостингами,  как <a href = "https://fotki.yandex.ru" target="_blank" >"Яндекс.Фотки"</a>,  <a href = "https://picasaweb.google.com" target="_blank" >"Picasa Веб-альбомы"</a>, <a href = "http://www.photohost.ru/" target="_blank" >"ФотоХост.Ру"</a> и т.п.</div>' +
			...
			formImageHtml += '</div></div>' +
				...
				'<div class="form-row"><label for="name">{width} x {height}:</label><div class="form-row-value"><input type="text" name="width" value="" class="width-small"/ disabled="disabled"> x <input type="text" name="height" value="" class="width-small"/ disabled="disabled"></div></div>' +
		...
		processInsert: function (context, Wysiwyg, img) {
			var image,
				...
				width = '150px',//$('input[name="width"]', context).val(),
