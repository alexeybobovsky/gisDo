...
/*	this.start = 	obj.dateStart;	
	this.dateEnd = 		obj.dateEnd;*/
	
	this.labelFoto = (obj[9] == 2) ? 'Последние фотографии стройки' :  'Текущее состояние стройки' ;	
	
//		"<div id='state'><strong>Статус стройки: </strong><span title='Показать все объекты в состоянии \"$[properties.state]\"' id= 'filter_state~$[properties.stateId]-material~1' class='state_$[properties.stateId] actionLink' onClick='showConstruction(this)'>$[properties.state]</span></div>" + 
	
	this.state = 	((obj[10] == '')||(obj[10] == 0)) ? '' : "<div id='state'><strong>Статус стройки: </strong><span title='Показать все объекты в состоянии \"" + obj[10] + "\"' id= 'filter" + obj[2] + "_state~" + obj.stateId + "' class='state_" + obj[9] + " actionLink' onClick='showConstruction(this)'>" + obj[10] + "</span></div>" 
...
function setPointConstruction(obj) //поля метки для стройки
	{
...
//	str.replace(/тест/g,"прошел")
    ...
    this.adr = obj.location;
	this.stateId = obj.stateId;	
	this.rate = obj.rate;	
	this.info = str_replace ( '<br />', '\n', obj.info );
	this.nameTrans = obj.nameTranslit;	
	this.material = obj.material;	
	this.materialId = obj.materialId;	
/*	this.start = 	obj.dateStart;	
	this.dateEnd = 		obj.dateEnd;*/
	
	this.labelFoto = (obj.stateId == 2) ? 'Последние фотографии стройки' :  'Текущее состояние стройки' ;	
	
//		"<div id='state'><strong>Статус стройки: </strong><span title='Показать все объекты в состоянии \"$[properties.state]\"' id= 'filter_state~$[properties.stateId]-material~1' class='state_$[properties.stateId] actionLink' onClick='showConstruction(this)'>$[properties.state]</span></div>" + 
	
	this.state = 	((obj.state == '')||(obj.state == 0)) ? '' : "<div id='state'><strong>Статус стройки: </strong><span title='Показать все объекты в состоянии \"" + obj.state + "\"' id= 'filter" + obj.objId + "_state~" + obj.stateId + "' class='state_" + obj.stateId + " actionLink' onClick='showConstruction(this)'>" + obj.state + "</span></div>" 
	this.material = ((obj.material == '')||(obj.material == 0)) ? '' : '<div id="mat"><strong>Материал: </strong><span title="Показать все объекты из этого материала" id= "filter' + obj.objId + '_material~' + obj.materialId + '" class="actionLink" onClick="showConstruction(this)">' + obj.material + '</span></div>'; 
	if((obj.dateStart == '')||(obj.dateStart == 0))
		{
		tmpArr = obj.monitoringStart.split(' ');
		tmpStr = tmpArr[0] + '#' + tmpArr[2];
		this.dateStart = '<div id="start"><strong>Начало мониторинга: </strong><span title="Показать все объекты,  строительство или наблюдение  за которыми началось в это время" id= "filter' + obj.objId + '_start~' + tmpStr + '" class="actionLink" onClick="showConstruction(this)">' + obj.monitoringStart + '</span></div>'; 
		}
	else
		{
		tmpArr = obj.dateStart.split(' ');
		tmpStr = tmpArr[0] + '#' + tmpArr[2];
		this.dateStart = '<div id="start"><strong>Начало строительства: </strong><span title="Показать все объекты, строительство или наблюдение за которыми началось в это время" id= "filter' + obj.objId + '_start~' + tmpStr + '" class="actionLink" onClick="showConstruction(this)">' + obj.dateStart + '</span></div>'; 
		}
	tmpStr = '';
	if((obj.dateEnd == '')||(obj.dateEnd == 0))
		{
		tmpArr = obj.monitoringStart.split(' ');
		tmpStr = tmpArr[0] + '#' + tmpArr[2];
		this.dateEnd = '<div id="end"><strong>Окончание мониторинга: </strong><span title="Показать все объекты, с такой же датой окончания строительства/наблюдения" id= "filter' + obj.objId + '_end~' + tmpStr + '" class="actionLink" onClick="showConstruction(this)">' + obj.monitoringEnd + '</span></div>'; 
		}
	else
		{
		tmpArr = obj.dateEnd.split(' ');
		tmpStr = tmpArr[0] + '#' + tmpArr[2];
		this.dateEnd = '<div id="end"><strong>Окончание строительства: </strong><span title="Показать все объекты, с такой же датой окончания строительства/наблюдения" id= "filter' + obj.objId + '_end~' + tmpStr + '" class="actionLink" onClick="showConstruction(this)">' + obj.dateEnd + '</span></div>'; 
		}
	tmpStr = '';
	this.foto = new Array();	
	this.dateLastFoto = obj.dateLastFoto;
	...
	if(obj.fotoInfo!='')		
		{
		tmpStr += '<div id="fotoInfo">';
		for(var i=0; i<obj.fotoInfo.length; i++)
		if (obj.fotoInfo[i] !='')
			{

			tmpArr = obj.fotoInfo[i].split('size/');
			fileName =  tmpArr[1].split('.');
			this.imgInfo[cnt] = new setpointImg(tmpArr[0], fileName[0], fileName[1]);
			
			title 	= 	'Фото информационной доски объекта '+ obj.name + ' город ' + $('#cityName').val();			
			imgLink = 	tmpArr[0] + imgSizeView + '/' + tmpArr[1];		
			if(i!=0)			
				galleryStrInfo += ',';			
			galleryStrInfo += '{href : \'' + imgLink + '\', title : \'' + title + '\'}';
	...
	if(obj.fotoRender!='')	
		{
		tmpStr += '<div id="fotoRender">';
		for(var i=0; i<obj.fotoRender.length; i++)
		if (obj.fotoRender[i] !='')
			{
			tmpArr = obj.fotoRender[i].split('size/');
			fileName =  tmpArr[1].split('.');
			this.imgRender[cnt] = new setpointImg(tmpArr[0], fileName[0], fileName[1]);
			
			title 	= 	'Рендер объекта '+ obj.name + ' город ' + $('#cityName').val();			
			imgLink = 	tmpArr[0] + imgSizeView + '/' + tmpArr[1];		
			if(i!=0)			
				galleryStrRender += ',';			
			galleryStrRender += '{href : \'' + imgLink + '\', title : \'' + title + '\'}';
	...
	if(obj.fotoScheme!='')		
		{
		tmpStr += '<div id="fotoSheme">';
		for(var i=0; i<obj.fotoScheme.length; i++)
		if (obj.fotoScheme[i] !='')
			{
			tmpArr = obj.fotoScheme[i].split('size/');
			fileName =  tmpArr[1].split('.');
			this.imgSheme[cnt] = new setpointImg(tmpArr[0], fileName[0], fileName[1]);						
			title 	= 	'Схема съёмки объекта '+ obj.name + ' город ' + $('#cityName').val();
			imgLink = 	tmpArr[0] + imgSizeView + '/' + tmpArr[1];		
			if(i!=0)			
				galleryStrSheme += ',';			
			galleryStrSheme += '{href : \'' + imgLink + '\', title : \'' + title + '\'}';
	...
	if(obj.fotoSet!='')
		{
		tmpStr += '<div id="fotoSet">';
		for(var i=0; i<obj.fotoSet.length; i++)
		if (obj.fotoSet[i] !='')
			{
			tmpArr = obj.fotoSet[i].split('size/');
			fileName =  tmpArr[1].split('.');
			this.foto[cnt] = new setpointImg(tmpArr[0], fileName[0], fileName[1]);

			title 	= 	'Фото c позиции ' + fileName[0] + '. Объект '+ obj.name + ' город ' + $('#cityName').val();
			imgLink = 	tmpArr[0] + imgSizeView + '/' + tmpArr[1];		
			styleAdd = '';
			tmpStr += '<a  class="imgBaloonLink"'+ styleAdd + '  rel="gallery" href = "' + imgLink + '" title = "' +  title 
												+ '" target="_blank"><img id = "imgBaloonPrew_' + tmpArr[1] + '" height="60" width="60" src="' 
...
//	if(this.imgRender[0] != 'undefined')
	if(this.imgRender.length > 0)
		...
		this.img[0] = this.foto[0];
	this.logo = this.img[0].path + '150/' + this.img[0].fileName
	tmpStr = '';
	if(mngActions)
		{		
		tmpStr += '<div id = "mngStr">';
		tmpStr += '<span class="mngAct" id="af"  onClick="showEdtBox(\'' + mngActions.af + obj.id + '\', 0)"><nobr>съёмка</nobr></span>';
		tmpStr += '<span class="mngAct" id="oe"  onClick="showEdtBox(\'' + mngActions.oe + obj.type + '/' + obj.id + '\', 0)"><nobr>изменить</nobr></span>';
		tmpStr += '<span class="mngAct" id="fe"  onClick="moveObj(\'' + obj.type + '\', \'' + obj.id + '\')"><nobr>переместить</nobr></span>';
		tmpStr += '<span class="mngAct" id="od"  onClick="delObj(\'' + mngActions.od + '\', 0)"><nobr>удалить</nobr></span></br>';
		tmpStr += '</div>';
		}
	this.mngSting = tmpStr;
...
/*	var city = document.getElementById('cityId').value;*/
	UI.pleaseWait();
	$.post("/spddl/", {type:spdlType, objType:objType, filter:filter, object:paramId, city:city}, function(str) 
		{
		UI.pleaseWait();
//		var cssShadow = (objType == 'construction') ? {'-webkit-box-shadow' : '0 5px 5px rgba(0, 0, 0, 0.5)', '-moz-box-shadow' : '0 5px 5px rgba(0, 0, 0, 0.5)', 'box-shadow':'0 5px 5px rgba(0, 0, 0, 0.5)'} : {'-webkit-box-shadow' : '', '-moz-box-shadow' : '', 'box-shadow':''} ;
		
		var titleStrArr = 	str.split('``');
		var titleStrParArr = 	titleStrArr[0].split('%%');
		parentObj = new setParentObj(titleStrParArr[0], titleStrParArr[1], titleStrParArr[1]);
		...
		if(strArr.length <= 1)
			showType = 'empty';		
		else if((strArr.length == 2) && (strArr[1].length>5))
			showType = 'single';
		else if((strArr.length == 2) && (strArr[1].length<5))
			showType = 'empty';
...
//						alert(showProperties(valArr, 'valArr'));
//						var obj = jQuery.parseJSON(strArr[i]);
//						pointC[pointCnt] =  new setPointConstruction(obj);
						...
						if(objType == 'firm') 
							...
							if(valArr.length>1)
								...
								/*
								point[pointCnt] =  new setPoint(valArr[1], valArr[0], valArr[5], valArr[4], valArr[2], valArr[3], valArr[6], valArr[7], valArr[8], layerArr, imgArr);
...
function showFirm(obj) //показать единичный объект
	{
...
//	alert(showProperties(filter, 'filter'));
	/*
	var constrState = GetStrPrt(obj.id, '_', 1);
	filter.state = constrState;*/
	clearMapObjects(); 
	NAV.setSelectedType('construction', 0,  parArr[0],  parArr[1]);
	$('#objType').val('construction');		
	$('#objId').val('');		
	getObjects('mapObjects', 'construction', '', $('#cityId').val(), 1); 
	}
...
//	NAV.setSelectedType('singleObj', '', '');
	NAV.setSelectedType(objType, objId, '', '');
//	alert('from testSpddl - curObjId = ' + objId + ';curObjType = ' + objType);	
	$('#objType').val(objType);		
	$('#objId').val(objId);		
...
function init() 
	...
	var tplBaloonConstruction = "<div id='baloonPad' style='height:400px;'><div id='firmName'>" + 
		"<a title = 'Профиль стройки в каталоге' href='/catalog/$[properties.firmName]'>$[properties.firmName]</a>" + 
		"</div>" + 
		"$[properties.orgSting]" + 
		"<div id='info'>" + 
		...
		"</div>" + 
		"<div id='historyLink'>" + 
		"<a title = 'Смотреть всю хронологию строительства в каталоге' href='/catalog/$[properties.firmName]'>История строительства</a>" + 
		"</div>" + 		
		"$[properties.mngSting]"+
		"";		
