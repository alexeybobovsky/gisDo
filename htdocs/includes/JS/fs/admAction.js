...
function showFotoSet(objId, setNum, setDate)
	{
/*	alert(showProperties(setDate, 'sel'));/*/
...
function selectFirm(str) 
	{	
	document.getElementById('firmId').value = str.extra[1];
	}
function selectConstr(str) 
	{	
	document.getElementById('constrId').value = str.extra[1];
	}
...
function updateStylesApartments()
	{

	$("#constrName").autocomplete('/spddl/',
		{
		minChars:1,
		lineSeparator:"##",
		cellSeparator:"**",
		...
		onItemSelect:selectConstr,
		extraParams:{type:"constrName"}
	//	cacheLength:0,
	//    autoFill:false
		}
		);	
...
//		curPar = GetStrPrt(fileUploadParam, del, indx)		
		if(document.getElementById('file_' + fileUploadParam[i].name))
			{
			uploader[cnt] = new createFileUploader('Загрузить ' + fileUploadParam[i].title, fileUploadParam[i].name, '/apartment/set/upload');
			cnt ++;
			}
		}
	}
