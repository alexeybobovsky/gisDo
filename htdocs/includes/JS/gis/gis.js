...
/*			balCont += '<p> type = ' + obj.type + ' </br>';
			balCont += ' state = ' + obj.state + ' </p>';
			balCont += ' lStr = ' + obj.lStr + ' </p>';
/*			balCont += (obj.spec) ?'<p> type = ' + obj.type + ' </p>': '';*/
...
//								console.log(' [ADD] '+obj.name + ' : ' + obj.id);
	...
	//							obj.getBounds().getNorth();
								_setBounds([obj.getBounds().getEast(),obj.getBounds().getWest(),
											obj.getSouth().getNorth(), obj.getBounds().getNorth()]);
								mymap.addLayer(obj);
								}
							else
								{
...
//			console.log(showProperties(ULayerIndex));
//			var 
...
//						objId = counter;
						...
						if((UObjects[objId].lStr.indexOf('_' + distrLayerPar) >= 0))  /*объекты - районы*/{
							...
							if(UObjects[objId].info == ''){
								...
								UObjects[objId].info = layerDistr.objCnt + ' объектов';
								UObjects[objId].objCnt = parseInt(layerDistr.objCnt);
								if((objCntMin<0)||(parseInt(layerDistr.objCnt)<objCntMin))
									objCntMin = parseInt(layerDistr.objCnt);
								if((objCntMax<0)||(parseInt(layerDistr.objCnt)>objCntMax))
									objCntMax = parseInt(layerDistr.objCnt);
							}
