...
function selectName(str) 
	...
	if(str.selectValue == 'street')
		showObject(str.extra[3], 1);
	document.getElementById('searchStr').value = 'Поиск';
...
function getIconsOfZoom()
	...
	this.update = function (zoom)
		...
		if(zoom > 15)
			...
			this.imageSize = 	[32,32];
			this.imageOffset = [-16,-16];
			}
		else if(zoom > 14)
			{
			this.imageHref = 	'/src/design/tmp/main/blue_bar.png';
			this.imageSize = 	[24,24];
			this.imageOffset = [-12,-12];
			}
		else if(zoom > 12)
			{
			this.imageHref = 	'/src/design/tmp/main/blue_dot.png';
...
//		FB.galleryItemsAdd(imgLink, title);
		...
		tmpStr += '<a  class="imgBaloonLink"'+ styleAdd + '  rel="gallery" href = "' + imgLink + '" title = "' +  title 
											+ '" target="_blank"><img id = "imgBaloonPrew_' + tmpArr[1] + '" src="' 
	...
	if(mngActions)
		...
		tmpStr += '<span class="mngAct" id="od"  onClick="delObj(\'' + mngActions.od + '\', 0)"><nobr>удалить объект</nobr></span></br>';
	...
	tmpStr += '<span class="orgAct" id="showAllOrgObjLink"  onClick="showOrgObjects(\'' + firmId + '\', 0)"><nobr>Все объекты</nobr></span></br>';
	tmpStr += '<span class="orgAct" id="go2profileLink"  	onClick="go2OrgProfile(\'' + firmId + '\', 0)"><nobr>Профиль организации</nobr></span>';
	tmpStr += '</div>';		
...
//		alert(showType);
		if(showType == 'empty')
			...
			DLG.showMessage('error', 'Объектов не найдено');			
			}		
		else
			{
...
function getObjects_(spdlType, paramId, city, show) //получение объектов с сервера и добавление их в геоколлекцию
	{
	var city = document.getElementById('cityId').value;
	DLG.pleaseWait();
	$.post("/spddl/", {type:spdlType, object:paramId, city:city}, function(str) 
		...
		if(show)
			{
			myMap.geoObjects.add(objCollection);
			objCnt = objCollection.getLength();
			if(objCnt > 1)
				myMap.setBounds(objCollection.getBounds(), { checkZoomRange: true,
				callback: function(err) {if (err) {alert('Ошибка масштаба - '+ myMap.getZoom());}}});
			else if (objCnt == 1)
				{
				myMap.setCenter(point[0].coordinates);	
				myMap.setZoom(16);
...
//	alert(showProperties(ymaps.getZoomRange('yandex#map', [52.289422, 104.280633]), 'zoom'));	
	...
	var tplMarkerBaloonStr = "<div id='baloonPad'><div id='firmName'>$[properties.firmName]</div>" + 
		"$[properties.orgSting]" + 
		"<div id='adrActions'>" + 
